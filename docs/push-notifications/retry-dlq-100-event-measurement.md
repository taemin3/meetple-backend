...
현재 기준 지연은 1 / 4 / 16 / 64초로 합계 85초다. 운영 설정에서는 jitter가 더해져 같은 시점에 실패한 이벤트의 재시도가 분산된다. 초당 1건 입력 시 입력 자체에 약 100초가 걸리므로 전체 실험이 85초에 끝나는 것은 아니다. 실제로는 jitter, 처리시간과 적체가 더해진다.
