...
- 인증번호 재발급 시 이전 challenge의 요청자별 실패 횟수를 초기화한다.
- 삭제 토큰에 발급 당시 회원 ID를 저장하고 최종 처리 시 같은 회원과 이메일인지 확인한다.
- DB 롤백 시 남은 TTL 범위에서 삭제 토큰을 복구한다.
- 참여 신청·취소 알림에는 닉네임을 새로 저장하지 않고 기존 닉네임 포함 알림을 익명화한다.
- 주최자 탈퇴로 취소된 모임의 승인 참여자에게 일반 취소 흐름과 같은 알림/Push Outbox를 생성한다.
- 일반 `MemberRepository.findById`에서 탈퇴 회원을 제외하되 익명화 기록의 연관 조회는 유지한다.
