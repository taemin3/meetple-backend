...
class ChatMigrationTest {
    ...
    void migrationsAdoptExistingSchemaAndCreateManagedTables() throws Exception {
        ...
        try (var connection = DriverManager.getConnection(url, "sa", "")) {
            ...
            assertThat(singleIntValue(connection, "select count(*) from categories"))
                    .isEqualTo(3);
    ...
    private int singleIntValue(java.sql.Connection connection, String sql) throws Exception {
        try (var statement = connection.createStatement();
             var resultSet = statement.executeQuery(sql)) {
            assertThat(resultSet.next()).isTrue();
