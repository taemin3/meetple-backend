...
public class LocalLegalDocumentInitializer implements ApplicationRunner {
    ...
    private String currentPrivacyPolicyContent() {
        return """
                ...
                - 위치 서비스: 이용자가 위치 권한을 허용하고 주변 모임 기능을 사용할 때 현재 좌표를 1회 조회하여 주변 모임 검색과 지도 표시에 이용합니다. 현재 위치를 회원 프로필에 상시 저장하거나 백그라운드에서 계속 수집하지 않습니다.
                ...
                - 데이터베이스 자동 백업: 재해 복구를 위해 최대 35일의 수명주기 안에서 보관될 수 있으며, 해당 기간에는 복구 목적 외로 이용하지 않고 수명주기 종료 후 자동 삭제합니다.

                탈퇴하면 이메일·비밀번호·닉네임·소개·지역·프로필 이미지는 삭제하거나 복구할 수 없도록 익명화합니다. 서비스 무결성을 위해 모임과 참여 상태, 채팅의 순서, 약관 확인 시각은 식별정보와 분리된 형태로 남을 수 있으며 채팅 본문과 참여 신청 메시지는 삭제 또는 대체됩니다. 관계 법령에 따라 별도 보존이 필요한 정보가 생기는 경우 해당 법령, 항목과 기간을 고지하고 다른 정보와 분리하여 보관합니다.
                ...
                5. 개인정보의 국외 이전
                푸시 알림 기능을 사용하는 경우 FCM 등록 토큰, 기기 플랫폼, 알림 제목·내용과 관련 모임 식별자가 암호화된 네트워크를 통해 미국을 포함하여 Google 또는 재위탁자가 처리 시설을 운영하는 국가로 전송될 수 있습니다. 이전은 토큰 등록과 알림 발송 시 발생하며 목적은 푸시 알림 제공입니다. Google의 보유 기간은 Firebase 서비스 정책에 따르며, Firebase 설치 식별자는 고객의 삭제 API 호출 후 실제 및 백업 시스템에서 삭제되기까지 최대 180일이 걸릴 수 있습니다. 회원 탈퇴 또는 토큰 무효화 시 밋플 서버의 FCM 토큰은 삭제됩니다. 푸시 알림은 기기 설정에서 거부할 수 있습니다.

                6. 파기 절차 및 방법
                보유 기간이 끝나거나 처리 목적이 달성된 개인정보는 지체 없이 파기합니다. 전자 파일은 복구하기 어려운 방식으로 삭제하고, 외부 저장소의 프로필 이미지는 삭제 작업을 예약해 제거합니다. 백업 데이터는 별도로 사용하지 않으며 정해진 수명주기에 따라 삭제합니다.
                ...
                8. 안전성 확보 조치
                밋플은 비밀번호 단방향 암호화, 전송 구간 암호화, 접근 권한 제한, 인증 토큰 만료와 폐기, 비밀정보 분리 보관, 로그 모니터링 등의 보호조치를 적용합니다.
